...
- (void)setKeyboardTriggerOffset:(CGFloat)keyboardTriggerOffset {
    ...
    objc_setAssociatedObject(self, &UIViewKeyboardTriggerOffset, @(keyboardTriggerOffset), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)setKeyboardActiveView:(UIView *)keyboardActiveView {
    ...
    if (keyboardActiveView) {
        [keyboardActiveView addObserver:self forKeyPath:@"frame" options:0 context:NULL];
    }
...
- (BOOL)isKeyboardOpened {
    return [objc_getAssociatedObject(self, &UIViewKeyboardOpened) boolValue];
}
...
- (BOOL)keyboardWillRecede {
    ...
    CGFloat thresholdHeight = keyboardWindowHeight - keyboardViewHeight - self.keyboardTriggerOffset + 44;
