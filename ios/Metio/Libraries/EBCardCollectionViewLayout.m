...
- (CGFloat)cardWidth {
    return self.collectionView.bounds.size.height - _offset.horizontal * 2;
}

- (CGFloat)cardHeight {
    return self.collectionView.bounds.size.height - _offset.vertical * 2;
}

- (CGFloat)pageWidth {
    return [self cardWidth] + _offset.horizontal / 2;
}

- (CGFloat)pageHeight {
    return [self cardHeight] + _offset.vertical / 2;
}
...
- (CGRect)frameForCardAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_layoutType == EBCardCollectionLayoutHorizontal) {
        CGFloat posX = _offset.horizontal / 2 + [self pageWidth] * indexPath.row;
...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity {
    ...
    if (velocityValue > 0.0) {
        currentPage = (NSUInteger)floor(rawPageValue);
    } else {
        currentPage = (NSUInteger)ceil(rawPageValue);
    }
    
    NSUInteger nextPage = 0;
    if (velocityValue > 0.0) {
        nextPage = (NSUInteger)ceil(rawPageValue);
    } else {
        nextPage = (NSUInteger)floor(rawPageValue);
    }
    ...
    if (pannedLessThanAPage && flicked) {
        ...
        if (_layoutType == EBCardCollectionLayoutHorizontal) {
            ...
            if ((NSInteger)nextPage < [self.collectionView numberOfItemsInSection:0]) {
        ...
        //  Bounces
        if (_layoutType == EBCardCollectionLayoutHorizontal) {
            CGFloat posX = (CGFloat)round(rawPageValue) * [self pageWidth] - _offset.horizontal/2;
            posX = MAX(0, posX);
            retVal.x = posX;
        } else {
            CGFloat posY = (CGFloat)round(rawPageValue) * [self pageHeight] - _offset.vertical/2;
            posY = MAX(0, posY);
            retVal.y = posY;
        }
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:@"collectionView.contentOffset"]) {
        ...
        NSUInteger newPage = (NSUInteger)round(floatPage);
